...
To deploy CTFd for a convention, clone this repo and run `git submodule init` and `git submodule update --remote --merge` to get the latest updates for LLM Verification. Then run `docker compose up` and navigate to [localhost:8000](http://localhost:8000).
